    ...
    <a-input placeholder="患者姓名" allowClear :maxLength="6" style="width:100px;"  v-model="filter.patient"/>
      ...
      <a-tab-pane key="1" tab="处方明细表">
        <a-table :columns="columns" :data-source="data"  bordered  size="small" rowKey="opertime" :expandRowByClick="true"
               :pagination="pagination" @change="handleTableChange"  @expand="expandInner" style="color:red;"
          :customRow="mycustom">
           <a-table style=""
      ...
      <a-tab-pane key="2" tab="统计明细表" force-render style="text-align: center;">
        选择分组依据:
        ...
        <a-radio-button :value="item.name" :checked="index==0">
          ...
          <a-table :columns="tjcols" :data-source="tongji"  size="small" :pagination="false">
          </a-table>
      </a-tab-pane>
    ...
    export default {
      ...
      data() {
        return {
          ...
          groupData: [
            {
              name: 'doctor', label: '医生', groupFn(item) {
                return item.doctor
              }
            },
            {
              name: 'opertimeByDay', label: '日期(按日)', groupFn(item) {
                return item.opertime.substr(0, 10)
              }
            },
            {
              name: 'opertimeByMonth', label: '日期(按月)', groupFn(item) {
                return item.opertime.substr(0, 7)
              }
            },
            {
              name: 'department', label: '科室', groupFn(item) {
                return item.department
              }
            },
            {
              name: 'gender', label: '性别', groupFn(item) {
                return item.gender
              }
            },
            {
      ...
      methods: {
        ...
        mycustom: function (record, index) {
          return {
            style: {
              color:'red',
              cursor:'hand'
            ...
            on: { // 事件
              ...
              contextmenu: (event) => {
                this.$message.error("hehe")
              },
              mouseenter: (event) => {
                console.log(this)
                this.$message.info(record.patient)
              },  // 鼠标移入行
              mouseleave: (event) => {
              }
            },
